function displayBoxes(sublesson) {
  ...
  if(lesson_id == 1){
    let lesson_img_container = $('<div class=col-md-6>');
    let lesson_text = $('<div class=col-md-6>');
    $(lesson_text).text(images_text[sublesson["sublesson_id"] - 1])
    let lesson_img = $('<img class="display-image" src="' + images[sublesson["sublesson_id"] - 1] + '">')
    $(lesson_img_container).append(lesson_img)
    $(".lesson-content").append(lesson_img_container)
    $(".lesson-content").append(lesson_text)
  }
  ...
  $.each(images, function (index, value) {
    ...
    $(new_shape).click(function (event) {
      window.location.href =
        "/learn/" + lesson_id + "/" + $(new_shape).attr("id");
    });
  });
}
...
function displayOptionsSkin() {
  ...
  $.each(options, function (index, value) {
    ...
    $(new_option).click(function (event) {
      window.location.href =
        "/learn/" + lesson_id + "/" + $(new_option).attr("id");
    });
  });
}
  ...
  $.each(sublesson["colors"], function (index, value) {
    if (index == 0) {
      ...
      $.each(sublesson["add_colors_1"], function (index, value) {
        let inner_shape = $('<div class="inner-shape-skin">');
...
function displayButtons() {
  ...
  $.get(
    "get_completed",
    { sublesson: sublesson["sublesson_id"] },
        ...
        if (!clicked.includes("False")) {
          ...
          $(next_button).click(function (event) {
            if (next_sublesson <= completed.length) {
              window.location.href =
                "/learn/" + lesson_id + "/" + next_sublesson;
            } else {
